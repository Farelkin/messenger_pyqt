...
class ClientSender(threading.Thread, metaclass=ClientMaker):
    def __init__(self, account_name, sock):
        """
        Класс формировки и отправки сообщений на сервер
    ...
    def print_help(self):
        ...
        print('message - отправить сообщение. Кому и текст будет '
              'запрошены отдельно.')
...
class ClientReader(threading.Thread , metaclass=ClientMaker):
    def __init__(self, account_name, sock):
        """
        Класс-приёмник сообщений с сервера. Принимает сообщения,
        выводит в консоль
        """
        self.account_name = account_name
        self.sock = sock
...
def create_presence(account_name):
    ...
    logger.debug(f'Сформировано {PRESENCE} '
...
def main():
    ...
    try:
        ...
        logger.error(f'При установке соединения '
                     f'сервер вернул ошибку: {error.text}')
        exit(1)
    except ReqFieldMissingError as missing_error:
        logger.error(f'В ответе сервера отсутствует '
                     f'необходимое поле {missing_error.missing_field}')
        exit(1)
